package com.HolidayTracker.fullstackbackend.auth;
...
public class AuthenticationHelper {
     ...
     */
    public JSONObject PerformSRPAuthentication(String username, String password) {
        ...
        if (ChallengeNameType.PASSWORD_VERIFIER.toString().equals(initiateAuthResult.getChallengeName())) {
            RespondToAuthChallengeRequest challengeRequest = userSrpAuthRequest(initiateAuthResult, password, null);
            RespondToAuthChallengeResult result = cognitoIdentityProvider.respondToAuthChallenge(challengeRequest);
            return CognitoJWTParser.getPayload(result.getAuthenticationResult().getIdToken());
        }
        throw new RuntimeException("Unexpected error aws cognito authenticating");
    }
